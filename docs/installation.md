...
Monozukuri runs as a CLI on top of the Claude Code CLI. It is shipped through three channels — Homebrew, NPX, and source. This page covers the requirements, the three install paths, how to verify the install worked, and platform-specific notes.
...
| `claude` | latest  | The Claude Code CLI that hosts the skill invoked for each feature |

If you install via Homebrew, `node`, `jq`, and `gh` are pulled in as Homebrew dependencies — you only need to install `claude` separately. The NPX and source paths assume the prerequisites are already present.

**Authentication.** `gh auth login` must be completed once per machine (or `GH_TOKEN` exported) before the PR phase will succeed. `claude` requires `ANTHROPIC_API_KEY` in the environment. See the [Claude Code documentation](https://docs.claude.com) for the current auth flow.
...
That gives you the `monozukuri` command on `PATH`, plus the prerequisites the formula declares as dependencies. Install the Claude Code CLI separately following its own documentation.
...
The NPX path does not install the `node` / `jq` / `gh` / `claude` prerequisites — you need those on your machine first.
...
`monozukuri doctor` checks each prerequisite, verifies `gh` and `claude` authentication, and reports the version of each tool. A green report means you can run `monozukuri init` in a project.

If `doctor` reports an issue, the most common causes are:

- **`claude` not found.** Install the Claude Code CLI per its documentation, then re-run.
