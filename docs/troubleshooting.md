...
**Circuit breaker.** The mechanism that terminates a phase after `phase_max_attempts` retries or `phase_budget_seconds` wall time. Selected loop mode also has a consecutive-failure circuit breaker controlled by `--circuit-breaker`; it exits `5` after the configured number of failed features in a row. Both prevent runaway loops in `full_auto` mode where there is no human gate. See [ADR-010](adr/010-stuck-state-elimination.md).
