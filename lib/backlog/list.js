...
Flags:
  ...
  --pick-format ids|json    Output mode for pick (default: json)
...
if (!['ids', 'json'].includes(pickFormat)) {
  process.stderr.write(`Invalid --pick-format: ${pickFormat || '(empty)'}\n`);
  process.exit(2);
}
...
try {
  ...
  const rows = ranked(items, { includeScore: pickMode });
  if (pickMode && pickFormat === 'ids') printIds(rows);
  else if (pickMode) printJson(rows);
