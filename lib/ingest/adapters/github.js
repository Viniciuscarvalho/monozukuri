...
try {
  raw = execSync(
    `gh issue list --label "${LABEL}" --state open --json number,title,body,labels,milestone,assignees,url,createdAt --limit 100`,
    { encoding: 'utf-8' }
  );
} catch (e) {
...
const items = issues.map(issue => ({
  ...
  effort: issue.labels?.find(l => l.name.startsWith('effort/'))?.name.replace('effort/', '') || '-',
  status: 'backlog',
