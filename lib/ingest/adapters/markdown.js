...
// lib/ingest/adapters/markdown.js
...
for (const line of lines) {
  ...
  const agentMatch = line.match(/^-\s*(agents?|compatible_agents|agent[-_ ]compatibility):\s*(.+)/i);
  if (agentMatch) {
    current.agents = agentMatch[2].split(',').map(a => a.trim()).filter(Boolean);
    continue;
  }

  const depMatch = line.match(/^-\s*(depends_on|depends?\s+on|depends?|dependencies):\s*([\w-,\s]+)/i);
  if (depMatch) {
    current.dependencies = depMatch[2].split(',').map(d => d.trim()).filter(Boolean);
    continue;
  }
