...
mem_refresh_env() {
  if [ "${ENV_REFRESH:-true}" = "true" ] && [ -f "${SCRIPTS_DIR:-}/environment-discovery.sh" ]; then
    bash "${SCRIPTS_DIR}/environment-discovery.sh" > "$CONTEXT_DIR/environment.manifest.json" 2>/dev/null || true
  fi
}
