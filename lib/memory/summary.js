...
function appendTrace(tracePath, payload) {
  if (!tracePath) return;
  try {
    ...
    // Memory tracing is diagnostic and must not block prompt construction.
  }
}
...
function contextEntries({phase, featureId, agent, learnings, cacheDir, cap, tracePath}) {
  ...
  appendTrace(tracePath, {
    ...
    phase,
    feature_id: featureId,
    entered: relevant.map((entry) => entry.id).filter(Boolean),
    included,
    omitted: result.available_on_request,
    tokens: tokenCount(result.summary_text),
    cache_hit: Boolean(result.cache_hit)
  });
  return out;
}

function rawEntrySummary(entry) {
  const source = entry.source && typeof entry.source === 'object' ? entry.source : {};
  const tags = Array.isArray(entry.tags) ? entry.tags.join(', ') : '';
  return [
    `<!-- learning: ${entry.id} --> raw_memory: ${entry.id}`,
    `insight: ${String(entry.insight || '').replace(/\s+/g, ' ').trim()}`,
    entry.rationale ? `rationale: ${String(entry.rationale).replace(/\s+/g, ' ').trim()}` : '',
    `scope: ${entry.scope || 'project'}`,
    `applied_count: ${Number.isInteger(entry.applied_count) ? entry.applied_count : 0}`,
    source.feature_id ? `source.feature_id: ${source.feature_id}` : '',
    source.phase ? `source.phase: ${source.phase}` : '',
    source.run_id ? `source.run_id: ${source.run_id}` : '',
    source.artifact ? `source.artifact: ${source.artifact}` : '',
    tags ? `tags: ${tags}` : ''
  ].filter(Boolean).join('\n');
}

function appendRawEscalations(out, learnings, idsCsv) {
  const ids = String(idsCsv || '').split(',').map((id) => id.trim()).filter(Boolean);
  if (ids.length === 0 || !Array.isArray(learnings)) return out;
  const byId = new Map();
  ...
  for (const id of ids) {
    ...
    seen.add(id);
    out.push({id, summary: rawEntrySummary(byId.get(id)), raw: true});
  }
  return out;
}
...
function main() {
  ...
  if (mode === 'summarize') {
    ...
    appendTrace(process.argv[7] || '', {
      event: 'summarize',
      phase,
      feature_id: featureId,
      entered: learnings.map((entry) => entry && entry.id).filter(Boolean),
      included: [...result.summary_text.matchAll(/<!--\s*learning:\s*([A-Za-z0-9_.:-]+)\s*-->/g)].map((match) => match[1]),
      omitted: result.available_on_request,
      tokens: tokenCount(result.summary_text),
      cache_hit: Boolean(result.cache_hit)
    });
  ...
  if (mode === 'context') {
    ...
    const escalationIds = process.argv[8] || '';
    const tracePath = process.argv[9] || '';
    const result = contextEntries({phase, featureId, agent, learnings, cacheDir, cap, tracePath});
    process.stdout.write(JSON.stringify(appendRawEscalations(result, learnings, escalationIds)));
    return;
  }
