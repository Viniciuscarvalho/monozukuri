...
# Source local-model.sh if not already loaded
_INJECTION_SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if ! declare -f local_model::classify &>/dev/null 2>&1; then
  if [ -f "$_INJECTION_SELF_DIR/local-model.sh" ]; then
    source "$_INJECTION_SELF_DIR/local-model.sh" 2>/dev/null || true
  fi
fi
