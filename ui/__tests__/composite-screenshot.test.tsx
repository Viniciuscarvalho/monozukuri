...
describe('composite — full TUI render with all five PRs applied', () => {
  it('captures the full Header + FeatureCard + footer-sep stack with live telemetry', () => {
    const INNER = 86;
    const Composite = () => (
      <Box flexDirection="column">
        <Header state={headerState} innerWidth={INNER} />
        <FeatureCard feature={activeFeature} now={NOW} innerWidth={INNER} />
        <Text color={tokens.dim}>{sep('q quit · p pause · ? help', INNER)}</Text>
      </Box>
    );
    ...
    expect(frame).toContain('2k');                        // tokensOut formatted
    expect(frame).toContain('4.2k/min');                  // tokenRate formatted
    expect(frame).toContain('recent activity');           // activity section
    expect(frame).toContain('Read');
    expect(frame).toContain('Edit');
    expect(frame).toContain('Bash');
    expect(frame).toContain('├─');                        // tree chars
    expect(frame).toContain('└─');                        // tree chars (last)
    expect(frame).toContain('q quit');                    // footer sep
